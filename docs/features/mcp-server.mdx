...
InfoAnalytica exposes a remote [Model Context Protocol](https://modelcontextprotocol.io) (MCP) server so external AI agents — Claude Code, Cursor, VS Code Copilot, Windsurf, and others — can work with the Postgres connections InfoAnalytica manages.
...
The server is mounted on the running InfoAnalytica instance over Streamable HTTP:
...
Every execution tool runs the submitted SQL through InfoAnalytica's parser-based permission detection before touching the database:
