...
InfoAnalytica provides fine-grained access control for your database connections. You define [IAM rules](/configuration/config#access-control-iam) in your configuration file, and InfoAnalytica enforces them — rejecting unauthorized queries before they reach the database.
...
Unlike PostgreSQL's built-in role system (`GRANT`/`REVOKE`), InfoAnalytica's access control operates at the application layer. You don't need to create database roles for each user — all users connect through shared credentials while InfoAnalytica enforces per-user permissions, controls application-level features like export, and logs every action for [auditing](/features/audit-log).
