...
InfoAnalytica uses a TOML configuration file for all settings. Pass the config file with the `--config` flag. Without `--config`, InfoAnalytica starts in [demo mode](/getting-started/quickstart#demo-mode).
...
| `vendor` | AI vendor: `openai`, `anthropic`, `google`, `openai-compatible`, `ollama-cloud` | Yes |
...
Ollama works two ways, depending on where the model runs:

- **Ollama Cloud** — `vendor = "ollama-cloud"` runs [Ollama's hosted models](https://ollama.com). `base_url` defaults to `https://ollama.com/v1`, so a provider needs only `model` and `api_key`. Create a key at [ollama.com/settings/keys](https://ollama.com/settings/keys).
- **Local Ollama** — `vendor = "openai-compatible"` with `base_url = "http://localhost:11434/v1"`. No `api_key`, since a local Ollama runs unauthenticated.
