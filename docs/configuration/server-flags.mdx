...
<Info>
`PGCONSOLE_CONFIG` contains the full TOML configuration as a string. When set, InfoAnalytica parses it directly instead of reading from a file. `--config` takes precedence over `PGCONSOLE_CONFIG`. This is useful for PaaS platforms (Railway, Cloudflare, etc.) where you cannot mount files.
</Info>
